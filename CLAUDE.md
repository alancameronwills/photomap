...
Keeps the map, POIs and photos displaying through mobile signal drop-outs on a walk. Three files, no build step:
...
- **`sw.js`** — service worker (scope `/`). Precaches the app shell on install (`Promise.allSettled` so one missing asset can't block). Fetch routing: tiles and photos are cache-first under the normalised keys (fetched `mode:'cors'` to avoid opaque-response quota padding; photos fall back to `no-cors`); `/api/pois|routes|projects` and `/auth/status` are network-first with a timeout, falling back to the cached copy and then to a synthesized empty response — so the startup `Promise.all` never rejects offline and the loading overlay always clears; other same-origin GETs are stale-while-revalidate; non-GET and other `/api/`/`/auth/` traffic passes through. A `{type:'trim'}` message triggers entry-count caps (tiles 2500 / photos 600 / api 40). `server.js` serves `sw.js` with `Cache-Control: no-cache` so deploys pick up promptly.
