...
- `addRouteNodes(routeId, nodes)` — bulk-populates a brand-new, empty route; assigns `order_index` 0..n-1 from array order instead of querying for the current max, so a GPX import's ~100+ sequential nodes can't collide on DynamoDB's eventually-consistent `route_id-order_index-index` GSI (the cause of a real "reversed pairs" bug — two nodes racing to the same `order_index` then sorting by UUID tiebreak instead of import order). Only valid for populating an empty route; `addRouteNode` is still used for single-node appends/prepends.
- `insertRouteNodes(routeId, insertions)` — bulk counterpart to `insertRouteNode`, for the same GSI-race reason as `addRouteNodes`: reads the route's nodes once, then bisects `order_index` for every insertion in memory (`insertions: [{afterNodeId, lat, lng, poiId}]`), stacking insertions that share an anchor in call order, and writes them all in one batch. Used by `POST /routes/:id/nodes/bulk` (the client's stray-POI auto-connect pass); a sequentially-awaited loop of single-node inserts against one route was found to carry the same race as the GPX import.
...
`POST /routes/:id/split` delegates to `db.splitRoute`. `POST /routes/:id/join` (body `{poiId}`, `:id` = the surviving route) delegates to `db.joinRoutes`, returning 409 when the two routes aren't joinable at that POI. `DELETE /route-nodes/:id` returns the full deletion result so the client can do surgical cleanup. `POST /routes/:id/nodes/bulk` (body `{insertions: [{afterNodeId, lat, lng, poiId}]}`) delegates to `db.insertRouteNodes` — see that function for why a request-per-node loop against DynamoDB isn't safe.
...
**Node-add serialization** — `handleRouteMapClick` and `addNodeAtPoi` (the map-click and POI-click paths for extending a route) don't await `addNodeToRoute`, so clicking rapidly while drawing a route could fire overlapping `POST /routes/:id/nodes` requests against the same route. Both call `queueAddNodeToRoute` instead, which chains every call onto one promise so only one is ever in flight — otherwise DynamoDB's `route_id-order_index-index` GSI query in `addRouteNode` could race a still-propagating write from the previous click and hand two nodes the same `order_index` (the same bug class as the GPX-import "reversed pairs" issue).
